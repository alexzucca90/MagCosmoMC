...
!MagCAMB: adding two more equations:
...
!MagCAMB: add initial_magnetic for magnetic initial conditions.
    !Supported scalar initial condition flags
    integer, parameter :: initial_adiabatic=1, initial_iso_CDM=2, &
        initial_iso_baryon=3,  initial_iso_neutrino=4, initial_iso_neutrino_vel=5, &
        initial_magnetic = 6, initial_vector = 0
    integer, parameter :: initial_nummodes =  initial_magnetic
...
!MagCAMB: adding magnetic indices
!integer :: mdi,mpi
...
!MagCAMB: added evolve_mag
logical :: evolve_mag = .true.
    contains
...
!MagCAMB: adding magnetic variables indeces.
!EV%mdi = EV%nvar+ 1
!EV%mpi = EV%mdi + 1
!EV%nvar= EV%nvar+ 2
    ...
    real(dl) ISW
!MagCAMB: adding magnetic Delta(a) and Pi(a)
real(dl) :: cdelb, cpib
...
!MagCAMB: adding magnetic contributions
    dgrho = dgrho + grhog_t*clxg+grhor_t*clxr + grhog_t*cdelb
    dgq   = dgq   + grhog_t*qg+grhor_t*qr
    dgpi  = dgpi  + grhor_t*pir + grhog_t*pig + grhog_t * cpib
...
!MagCAMB: replacing intv first argument dimension
real(dl) initv(7,1:i_max), initvec(1:i_max)
...
!Magnetic: 
    ...
    real(dl) yt(EV%nvart)
!MagCAMB: tens0 is defined somewhere else
...
!MagCAMB: mods from Shaw's code.
    yt(2) = tens0 -15/28._dl*x**2*(bigR-1)/(15+4*bigR)*pib*(1-5./2*omtau/(2*bigR+15))
    !yt(2)= tens0
    ...
    !shear
    yt(3)=-5._dl/2/(bigR+5)*x*elec + 15._dl/14*x*(bigR-1)/(4*bigR+15)*pib*(1 - 15.*omtau/(2*bigR+15))
    ...
    !  Neutrinos
    if (DoTensorNeutrinos) then
        pir=-2._dl/3._dl/(bigR+5)*x**2*elec - (1.0_dl - bigR)/bigR*pib*(1-15./14*x**2/(15+4*bigR))
        !           + (bigR-1)/bigR*Magnetic*(1-15./14*x**2/(15+4*bigR))
        aj3r=  -2._dl/21._dl/(bigR+5)*x**3*elec - 3._dl/7*x*(1.0_dl - bigR)/bigR*pib!&
    ...
    yv(2)= vec_sig0*(1- 15._dl/2*omtau/(4*bigR+15)) + 45._dl/14*x*pib*(BigR-1)/(4*BigR+15) &
    - 675._dl * (BigR - 1) * pib / (28._dl * (15+2*BigR)*(15+4*BigR)) * omtau * x

!MagCAMB: modifying following Shaw...
...
+ x**2*vec_sig0/6/BigR +0.5_dl*x*(1/bigR-1)*pib
    !yv((EV%lmaxv-1+1)+(EV%lmaxpolv-1)*2+3+1) = -1._dl/3*vec_sig0*(4*BigR+5)/bigR &
    !+ x**2*vec_sig0/6/BigR +0.5_dl*x*(1/bigR-1)*Magnetic

    !pi_r

pir=-2._dl/3._dl*x*vec_sig0/BigR - (1/bigR-1)*pib*(1._dl &
- 45._dl / (14._dl * (15+4*bigR)) * x**2)

    !pir=-2._dl/3._dl*x*vec_sig0/BigR - (1/bigR-1)*Magnetic
    yv((EV%lmaxv-1+1)+(EV%lmaxpolv-1)*2+3+1 +1)=pir
    yv((EV%lmaxv-1+1)+(EV%lmaxpolv-1)*2+3+1 +2)=3._dl/7*x*pib*(1-1/BigR)
...
!MagCAMB: adding magnetic variables
! Variables for current magnetic field values
real(dl) cdelb, cpib
real(dl) va, bs, rho_cgs ! Alfven velocity
! Variables for some of the plasma physics
real(dl) kc,bc
real(dl) Rv, Rg, Rb, Rc, omega

!MagCAMB: add constants
real(dl), parameter :: const_pi = 3.1415926535897932384626433832795_dl
real(dl), parameter :: G=6.6738e-11_dl
real(dl), parameter :: kappa=8._dl*const_pi*G
real(dl), parameter :: Mpc = 3.085678e22_dl
real(dl), parameter :: c = 2.99792458e8_dl !yun

Rv = grhornomass / (grhornomass + grhog)
Rg = 1- Rv
Rb = grhob / (grhob+ grhoc)
Rc = 1 - Rb
omega = (grhob+grhoc)/sqrt(3*(grhog+grhornomass))
...
!MagCAMB:
!if(blambda > 0._dl .and. evolve_mag) then
!
!bs = (blambda * 1d-9) * (k / klambda)**((3._dl+nb_ind)/2._dl) ! Scaled magnetic field at current k
!rho_cgs = (grhob / a**3) * (c**2 / (kappa * Mpc**2)) / 1d3 ! Last factor is conversion to CGS
!va = bs / (4*pi* a**4 *rho_cgs)**0.5 / (c*1d2) ! Dimensionless Alfven velocity (i.e. c = 1)

! Add in the magnetic contribution to the sound speed (see
! paper for 2/9 factor)
!cs2 = cs2 + 2.0* va**2 / 9.0
...
+k/4*pb43*(clxg-2*pig-(2*cpib-cdelb)))/(1+pb43)
!        vbdot=(-adotoa*vb+cs2*k*clxb  &
...
!MagCAMB: changing qgdot
qgdot=4._dl/3*(-vbdot-adotoa*vb+cs2*k*clxb)/pb43 &
+k*(clxg-2*pig)/3 - k*(2*cpib- cdelb)/3._dl
        !qgdot=4._dl/3*(-vbdot-adotoa*vb+cs2*k*clxb)/pb43 &
...
! Set mag field derivatives
! At the moment using a crude approximation to the field damping.
!if(evolve_mag .and. a < 1._dl/(1100._dl + 1._dl)) then
!ayprime(6) = -2._dl *va**2 * k**2 / opacity * cdelb
!ayprime(7) = -2._dl *va**2 * k**2 / opacity * cpib
! To conserve energy, assume the energy from magnetic fields is dumped into photons+baryons
!ayprime(4) = ayprime(4) - ayprime(6) / (1._dl + 1._dl / photbar)
!ayprime(EV%g_ix) = ayprime(EV%g_ix) - ayprime(6) / (1._dl + 1._dl / photbar)
!else
ayprime(6) = 0._dl
ayprime(7) = 0._dl
!end if
...
(vbdot+adotoa*vb)/photbar - 0.5_dl*k*pib
       ! yvprime(4)=-0.5_dl*k*pig - &
...
!MagCAMB: adding magnetic contribution
rhopi=grhog_t*pig+grhor_t*pir+ grhog_t*pib
