...
if(CosmoSettings%scalar_compensated) then
    ...
    Info%ScalarCompTransfersDD%Params%InitPower%rat(1) = 1._dl  !yun
    Info%ScalarCompTransfersDD%Params%InitPower%n_run(1) = 0._dl
    Info%ScalarCompTransfersDD%Params%InitPower%nt_run(1) = 0._dl!yun
...
!Setting up k_D
    ...
    if(CMB%magind .ge. -1.5) then !n >= -1.5
...
MagneticMode = scalar_comp_dp
    ...
    Info%ScalarCompTransfersDP%Params%InitPower%rat(1) = 1._dl  !yun
    Info%ScalarCompTransfersDP%Params%InitPower%n_run(1) = 0._dl
    Info%ScalarCompTransfersDP%Params%InitPower%nt_run(1) = 0._dl!yun
...
!write(*,*) "------ Scal-Comp Pi-Pi -------"
 call this%SetCAMBInitPower(Info%ScalarCompTransfersPP%Params,CMB,1)
    ...
    !Info%ScalarCompTransfersPP%Params%InitPower%an(1) = camb_ind
...
MagneticMode = vector_comp
    ...
    Info%VectorTransfers%Params%InitPower%rat(1) = 1._dl  !yun
    Info%VectorTransfers%Params%InitPower%n_run(1) = 0._dl
    Info%VectorTransfers%Params%InitPower%nt_run(1) = 0._dl!yun
...
!Setting up k_D
    ...
    if(CMB%magind .ge. -1.5d0) then
        ...
        Info%VectorTransfers%Params%InitPower%CorrType = 4 !VECTOR
...
if(CosmoSettings%scalar_passive .or. CosmoSettings%tensor_passive) then
    ...
    Info%Transfers%Params%InitPower%rat(1) = 1._dl  !yun
    Info%Transfers%Params%InitPower%n_run(1) = 0._dl
    Info%Transfers%Params%InitPower%nt_run(1) = 0._dl!yun
...
!Setting up k_D
    ...
    !Set them to .true. according to which mode we want to compute
	...
	delb = 0._dl ! Set up perturbations
        ...
        if(CMB%magind.ge.-1.5d0) then
            ...
            Info%Transfers%Params%InitPower%CorrType = 5
    ...
    if(CosmoSettings%tensor_passive) then
        ...
        if(CMB%magind<-1.5) then
            if(Feedbacklevel>1) write(*,*) "USING INTERPOLATION TABLE"
            Info%Transfers%Params%InitPower%ant(1) = 2._dl*(3 + CMB%magind)
            Info%Transfers%Params%InitPower%TensorPowerAmp(1) = mag_psamp(CMB%magind, CMB%magamp, 5) * (6._dl*Rg*(lrat + (5._dl/(8._dl*Rv) - 1)))**2!yun
            ...
            Info%Transfers%Params%InitPower%CorrType = 0
        else if (CMB%magind .ge. -1.5) then
