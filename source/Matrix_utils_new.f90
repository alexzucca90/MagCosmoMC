...
!
! Returns -Log Likelihood for Gaussian: (d^T Cov^{-1} d
!Cov better be fixed
! Cov is already the cholesky factorization of the covariance matrix

function Matrix_GaussianHalfChisqCholDouble(Cov, d) result(LogLike)
    double precision, intent(inout):: Cov(:,:)
    double precision, intent(in):: d(:)
    double precision, allocatable :: tmp(:)
    double precision :: LogLike
...
end function Matrix_GaussianHalfChisqCholDouble
